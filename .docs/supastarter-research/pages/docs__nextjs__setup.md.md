---
url: https://supastarter.dev/docs/nextjs/setup.md
...
supastarter uses [Prisma](https://www.prisma.io/) as an ORM (database access layer). This means you can use any database supported by Prisma, including PostgreSQL, MySQL and MongoDB. You can find all supported databases [here](https://www.prisma.io/docs/reference/database-reference/supported-databases).
...
To create a new supastarter project all you need to do is run the following command _(replace `my-awesome-project` with the name of your project)_:
...
The Next.js codebase contains separate apps for each main surface:
...
Make sure you have installed [pnpm](https://pnpm.io/) before running the following command:
...
_If you are using Supabase, you want to follow our [Supabase setup guide](/blog/supabase-integration) as you will need a second connection string for the database migration._
...
Read more about how to use the ORMs in the [database documentation](/docs/nextjs/database/overview).
...
```ts copy title="packages/mail/provider/index.ts"
export * from "./resend";
...
See the [mailing documentation](/docs/nextjs/mailing/overview) for more information on how to set up your mail provider.

#### Set up the payment provider

Next, we need to set the payment provider which is used for handling payments:

```ts copy title="packages/payments/provider/index.ts"
export * from "./stripe";
...
[Storage setup guide](/docs/nextjs/storage/overview)

</Step>
<Step>
### Set up your local development environment (optional)

If you want to set up a complete local development environment, you can follow the [local development guide](/docs/nextjs/codebase/local-development).
