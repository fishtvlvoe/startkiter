...
- [x] 5.1 確認 `course-lifecycle-email` 完成 `/spectra-apply`：訂閱到期等生命週期通知事件觸發正確，獨立 CR PASS 且 0 Critical。程式碼先前已 merge（`7ed18a1a`）。本 session 重跑 focused tests：welcome 7、expiration 4、cron 4，共 15/15；`spectra analyze` 四維 Clean、`validate` 通過。獨立 CR：Critical 0／High 3／Medium 9／Low 5，PASS。報告：`openspec/changes/course-lifecycle-email/code-review.md`。task 5.2 真實買斷+信箱 e2e 仍未重跑（與封存副本相同缺口）。
