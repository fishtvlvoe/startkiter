...
- [x] 1.1 在 `packages/course/components/interactive/` 實作 `InstantQuiz.tsx`（即時選擇題與解析）
- [x] 1.2 在 `packages/course/components/interactive/` 實作 `ConceptCompare.tsx`（多 Tab / 前後代碼對照）
- [x] 1.3 在 `packages/course/components/interactive/` 實作 `TimelineSync.tsx`（時間軸連動外框容器）
- [x] 1.4 在 `packages/course/components/interactive/` 實作 `WorkflowSorter.tsx`（步驟拖曳排序）
- [x] 1.5 在 `packages/course/components/interactive/` 實作 `MicroSandbox.tsx`（即時參數微沙盒）
- [x] 1.6 在 `packages/course/components/interactive/` 實作 `TeacherAvatar.tsx`（互動式教師人像）
- [x] 1.7 在 `packages/course/components/interactive/` 實作 `DialogueWindow.tsx`（動態對話視窗）
- [x] 1.8 撰寫互動積木單元測試 (`packages/course/components/interactive/*.test.tsx`)
...
- [x] 3.2 實作影音時間軸監聽與 `TimelineSync` 滾動連動邏輯 (`useTimeSync` hook)
