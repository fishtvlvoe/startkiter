...
The route `/marketplace` SHALL render a list of enabled Plugins sourced from the `MOUNT_POINTS` static registry defined in `packages/platform`. Each listed Plugin MUST show its `name`, `version`, and enabled status. The page MUST NOT provide install/uninstall action buttons, file upload forms, or any endpoint that accepts arbitrary Plugin packages. In the git-push-auto-deploy model, module installation is performed by AI tools modifying the buyer's repository code, not through the Marketplace UI.
