...
- [x] 1.5 [P] 為訂閱期款 webhook（`apps/saas/app/api/payuni/period-notify/route.ts`）新增兩個紅燈測試案例，涵蓋 Requirement「Subscription records reserve invoice fields without implementing invoicing」新增的 Scenario 與 Requirement「Invoice issuance failure does not block payment success」：(a) 啟用＋自動開立時期款成功建立 `Invoice{subscriptionId, periodNumber}`，`periodNumber` 對應 PayUNi `PeriodOrderNo` 的實際期數；(b) 開票失敗時訂閱仍轉為 ACTIVE／`paidPeriods` 仍遞增，`Invoice{status: FAILED}`。驗證目標：`pnpm --filter @startkiter/saas test period-notify.test.ts` FAIL
...
- [x] 7.1 在 `packages/api/modules/course/lib/invoice-events.ts` 實作 `triggerInvoiceForSubscriptionPeriod(subscriptionId, periodNumber)`，邏輯同 `triggerInvoiceForOrder`；修改 `apps/saas/app/api/payuni/period-notify/route.ts`，在訂閱狀態 transaction 內以 PayUNi `PeriodOrderNo` 的實際期數建立可恢復的 `PENDING` 發票意圖，並以 subscription advisory lock 與該期數更新 `paidPeriods`；transaction 結束後再呼叫 `triggerInvoiceForSubscriptionPeriod`，外部開票不占用 DB transaction，失敗保留 `FAILED`，重送 webhook 或 stale operation 可重試。開票呼叫失敗不得影響既有訂閱狀態更新已完成的結果。驗證目標：task 1.5 全數轉綠燈
...
- [x] 11.2 派 Codex 或等效工具對本次全部 diff（task 2-10）做 Code Review（correctness／security／performance 三角度）：correctness 確認 CHECK 約束、唯一鍵、PayUNi 期數與發票 operation 狀態行為；security 確認電子發票金鑰不會被記錄進任何 log、`SETTINGS_ENCRYPTION_KEY` 解密失敗時 fail-closed（視為未設定，不拋出可能洩漏金鑰片段的錯誤訊息）、未登入或非 operator 呼叫作廢/折讓操作皆被拒絕；performance 確認外部開票／作廢不占用 DB transaction，webhook 可先完成付款狀態回應。驗證方式：第四輪發現的 2 個 High 已修正；第五輪發現的 Shopline 金額單位 High 已修正；第六輪全新 context CR 最終 `Critical 0 / High 0 / Medium 0`、Verdict `PASS`，詳見 `code-review.md`。
- [x] 11.3 用 ego-browser skill 跑一次完整 e2e：登入買家帳號 → 結帳（一次買斷）時選擇公司發票（填統編抬頭）→ 送出訂單 → 以測試金鑰模擬付款成功通知 → 確認訂單頁看到「已開立」發票狀態與正確的發票號碼格式 → 操作員在訂單頁對該筆發票點「作廢」成功 → 再跑一次訂閱結帳流程確認同樣能看到 `Invoice` 建立。驗證目標：截圖記錄每個關鍵畫面（結帳發票選擇、訂單頁發票狀態、作廢後狀態），過程中任何一步失敗即視為本 task 未完成
...
- [x] 11.5 逐項核對 design.md Implementation Contract 的 Acceptance criteria 與 Scope boundaries 是否全部滿足：以不輸出秘密的 `DATABASE_URL` dotenv wrapper 跑 API（48 files／209 tests）、SaaS（39 files／195 tests）、payments（17 files／76 tests），四個 package type-check、SaaS／marketing production build 皆 exit 0；`spectra validate` 兩張 change 均 valid，`git diff --check` exit 0，`packages/payments/factory.ts`／`packages/api/modules/course/lib/subscription-gateway.ts` 未被修改。驗證目標：上述可寫入環境的實跑結果全數 exit code 為 0

## 12. 需要 Fish 親自操作的部分（提前告知，不等最後）

- [x] 12.1 **已由 Fish 明確授權代理執行隔離 smoke test**：ezPay 正式串接金鑰已由 Fish 提供，並在隔離 E2E clone 的 operator「台灣統一發票」設定頁寫入加密 `SiteSetting`、關閉測試模式、開啟自動開立與電子發票總開關；使用正式 API 完成 NT$390 訂閱期款發票 `DQ70632357` 的開立與立即作廢，截圖與 DB／正式 API 查詢證據見 `e2e.md`。這只證明正式 API smoke test，不代表正式部署或正式資料庫上線驗收；正式資料庫沒有被切換。
