...
- [x] 4.1 派 Codex 或等效工具對本次全部 diff（task 1-3）做 Code Review（correctness／security／performance 三角度）：correctness 確認 `watchedSec` 只取較大值更新的邏輯正確（GREATEST 語意）、與 `course-video-watermark` 同時修改 `FluentPlayer.tsx` 的變更沒有互相覆蓋（apply 前先 `git diff` 確認浮水印的 `watermark` prop 仍完整存在）；security 確認 `recordWatchTime` 只能回報呼叫者自己的觀看記錄，不能代寫其他 userId；performance 確認每 30 秒一次的回報頻率不會對資料庫造成過量寫入（單一 upsert，不是每次都新增一筆記錄）。驗證方式：CR 報告 Critical 數量為 0（PM 覆核）。證據：`docs/verification/lesson-watch-time-tracking/13-code-review.md`，第三輪 CR commit `2a4925e6` 的 Critical／High／Medium／Low 皆為 0。
