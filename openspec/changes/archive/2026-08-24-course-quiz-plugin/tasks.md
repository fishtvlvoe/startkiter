...
- [x] 6.2 派 Codex 或等效工具對本次全部 diff（task 1-5）做 Code Review（correctness／security／performance 三角度）：correctness 確認四種題型的計分邏輯符合預期、security 確認 `showAnswers: false` 時作答頁與其對應 API 回應都不洩漏正確答案內容（不只是前端隱藏，後端 payload 本身不能含答案）、非本人無法讀取他人 `QuizAttempt`，performance 確認 `/quiz/[pluginContentId]` 頁面查詢沒有 N+1。驗證方式：CR 報告 Critical 數量為 0（PM 覆核）
