...
- [x] 5.1 執行全域測試（`platform`／`api`／`saas` 三個 package 的 `pnpm test`）與 `pnpm type-check`，全部通過。驗證：platform 19 files/93 tests passed；api 52 files/230 tests passed；saas 45 files/221 tests passed；`pnpm type-check` 27 successful/27 total（api/saas 使用 `pnpm exec dotenv -c --` 載入既有 `.env`）
