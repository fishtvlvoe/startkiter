...
- [ ] 50.7（2026-08-21 更新：task A/B 合併後原本 6/7 失敗剩 2/7）`nav-menu-items.test.ts` 兩個測試斷言還停在舊版精簡 MOUNT_POINTS（`renders all MOUNT_POINTS menu items sorted by order` 斷言 learner 只看到「課程」一項、`9.3 wide viewport` 斷言 fixed.length 為 1），MOUNT_POINTS 擴充新增 start/chatbot/settings 項目後這兩個斷言沒跟著更新，不是功能壞掉，是測試期望值過時，需要照新的 MOUNT_POINTS 實際內容重寫斷言
