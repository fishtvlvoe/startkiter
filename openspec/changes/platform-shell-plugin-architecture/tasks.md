...
- [x] 42.1 2026-08-23 直接查生產環境資料庫解決，不需要裁決：`psql` 對 Coolify 上的 `startkiter-saas-db` 查詢 `"order"`（0 筆）、`github_kit_grants`（0 筆）、`"user"`（1 筆，僅 Fish 自己的帳號）——目前沒有任何已用舊共用 pull-only 模式完成履約的真實買家，遷移排程無對象可遷，這題視同不適用而非「裁決不遷」，等未來真的有買家用舊模式履約時再重新評估
