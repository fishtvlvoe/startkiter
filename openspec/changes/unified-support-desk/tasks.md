...
- [~] 3.4（2026-08-22 新增，依賴 6.15 完成後才能做；**網域已改用 `app.startkiter.dev`，不再用 `startkiter.aiver.me`**——`.aiver.me` 是 Fish 個人網域，`startkiter.dev` 才是產品自己的網域，DNS A 記錄早就指到 `startkiter-managed-fleet-01`，之後全部參照都改用這個）在 Chatwoot 後台（Settings → Integrations → Webhooks）建立 webhook：URL `https://app.startkiter.dev/api/support/webhook/chatwoot?token=<CHATWOOT_WEBHOOK_SECRET>`，訂閱「對話已建立 (conversation_created)」「訊息已建立 (message_created)」。**Chatwoot 端設定已完成**（webhook 名稱 StartKiter AI Copilot，已建立成功）。驗證方式「觸發測試訊息確認伺服器回傳 200」**做不了**：`app.startkiter.dev` 目前沒有任何 Coolify 應用程式在跑（DNS 記錄存在但沒有部署目標），`apps/saas` 從沒被部署過，見新增的 3.5
- [ ] 3.5（2026-08-22 新增，卡關等 Fish 裁決）`apps/saas`（含這次改的 support API）需要實際部署到 Coolify 才能讓 webhook 端對端可測——目前 Coolify `startkiter-test` project 只有 3 個資源：nginx demo、`startkiter-coolify-git-deploy-test`（純部署機制測試，非真正跑 apps 的地方）、Chatwoot，沒有任何資源在跑 apps/saas。需要 Fish 決定要不要現在建立這個部署（走 Coolify git deploy，跟 Chatwoot 一樣裝在 `startkiter-managed-fleet-01`，網域用已經有 DNS 記錄的 `app.startkiter.dev`），還是先在本機驗證邏輯正確就好，部署留到之後
