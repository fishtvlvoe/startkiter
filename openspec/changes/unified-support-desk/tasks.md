...
- [ ] 1.2 確認 Fish 已針對 design.md Open Questions 中「沒有 BuyerDeployment 記錄的買家怎麼處理」給出裁決：驗證方式為對應決定已補寫進 design.md 或本檔案，且第 5 節任務依裁決結果調整範圍

## 2. 資料模型：`SupportTicket`（對應設計決策「`SupportTicket` 強關聯 `BuyerDeployment`（NOT NULL 外鍵）」）

- [ ] 2.1 撰寫紅燈測試：`SupportTicket` 插入缺少 `buyerDeploymentId` 時資料庫拒絕寫入（NOT NULL 外鍵約束），對應 Requirement「Ticket-to-deployment linkage」Scenario「Database constraint enforcement」，驗證方式為 `pnpm --filter database test` 出現預期失敗訊息（外鍵約束違反）
- [ ] 2.2 撰寫紅燈測試：對同一 `chatwootConversationId` 重複建立 `SupportTicket` 會違反 unique 約束，對應 Requirement「Ticket-to-deployment linkage」Scenario「Chatwoot conversation deduplication」，驗證方式為對應測試檔案跑出預期失敗
- [ ] 2.3 在 `packages/database/prisma/schema.prisma` 新增 `SupportTicket` model 與 `SupportTicketChannel`／`SupportTicketStatus`／`SupportTicketResolvedBy` enum（依 design.md Implementation Contract 的 Prisma schema），執行 `pnpm --filter database db push` 使第 2.1、2.2 節測試轉綠燈，驗證方式為 `pnpm --filter database test` 全綠且 `psql` 查詢 `\d "SupportTicket"` 確認索引與外鍵存在
...
- [ ] 4.1 撰寫紅燈測試：`LINE_MESSAGING_CHANNEL_ACCESS_TOKEN` 未設定時，LINE 頻道不出現在任何買家介面選項中，對應 Requirement「LINE channel ingestion」Scenario「LINE Messaging Channel not configured」，驗證方式為對應元件測試斷言 LINE 按鈕不渲染
- [ ] 4.2 撰寫紅燈測試：`TELEGRAM_BOT_TOKEN` 未設定時，Telegram 頻道不出現在任何買家介面選項中，對應 Requirement「Telegram channel ingestion」Scenario「Telegram Bot not configured」，驗證方式同 4.1
- [ ] 4.3 撰寫紅燈測試：LINE webhook 簽章驗證失敗回傳 401 且不建立/修改 `SupportTicket`，對應 Requirement「LINE channel ingestion」Scenario「Invalid LINE webhook signature」，驗證方式為 API 測試斷言回應碼與資料庫無異動
- [ ] 4.4 撰寫紅燈測試：Telegram webhook secret 驗證失敗回傳 401 且不建立/修改 `SupportTicket`，對應 Requirement「Telegram channel ingestion」Scenario「Invalid Telegram webhook secret」，驗證方式同 4.3
- [ ] 4.5 申請 LINE Messaging Channel（企業審核，儘早送出，與第 1 節前置依賴等待期平行進行）並記錄申請進度，驗證方式為 LINE Developers 後台顯示 Channel 已核准
- [ ] 4.6 申請 Telegram Bot（`@BotFather` `/newbot`），驗證方式為取得 Bot Token 並成功呼叫 `getMe` API 回傳 Bot 資訊
- [ ] 4.7 實作 LINE Messaging Webhook 接收邏輯（簽章驗證 + 訊息轉發進 Chatwoot），使第 4.1、4.3 節測試轉綠燈，對應 Requirement「LINE channel ingestion」Scenario「Buyer sends a LINE message」，驗證方式為 `pnpm test` 全綠 + 手動發送一則 LINE 訊息確認出現在 Chatwoot 收件匣
- [ ] 4.8 實作 Telegram Bot Webhook 接收邏輯（secret 驗證 + 訊息轉發進 Chatwoot），使第 4.2、4.4 節測試轉綠燈，對應 Requirement「Telegram channel ingestion」Scenario「Buyer sends a Telegram message」，驗證方式同 4.7 但改用 Telegram 手動測試
...
- [ ] 7.1 撰寫紅燈測試：買家帳號無 `BuyerDeployment` 記錄時，全站浮動客服框不建立 `SupportTicket` 並導向既有 email 客服說明，對應 Requirement「Ticket creation from the site-wide support widget」Scenario「Buyer has no BuyerDeployment record」，驗證方式為元件測試斷言顯示對應文案且未呼叫建單 API
