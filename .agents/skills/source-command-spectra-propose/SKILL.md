---
name: "source-command-spectra-propose"
description: "Migrated source command `spectra-propose`"
---

# source-command-spectra-propose

Use this skill when the user asks to run the migrated source command `spectra-propose`.
...
1. **Determine the requirement source**
   ...
   - Check if the conversation context mentions a plan file path (plan mode system messages include the path like `~/.Codex/plans/<name>.md`)
   - If found, check if the file exists at `~/.Codex/plans/`
