---
title: Store data for organizations
description: Learn how to store data for organizations in your supastarter application.
---
...
We are re-using the [post example](/docs/nextjs/api/define-endpoint) from the API documentation and extend it to store the organiaztion with it.
Let's assume you want to enable all members of an organization to edit the posts of the organization instead of just the author.
...
model Post {
  ...
  organizationId String // [!code ++]
  organization Organization @relation(fields: [organizationId], references: [id]) // [!code ++]
}
...
```typescript copy title="packages/api/src/routes/posts/router.ts"
import { z } from "zod";
...
export const postsRouter = new Hono().basePath("/posts").post(
  "/",
  authMiddleware,
  validator(
    "json",
...
This resolver is protected by the `authMiddleware` which means only authenticated users can access it.
...
```tsx
export const createOrganizationPostMutationKey = [
  "organization",
  "create-posts",
  "create",
] as const;
...
When using this mutation, you need to pass the `organizationId` that you can get from the `useActiveOrganization` hook.
...
```typescript copy title="packages/api/src/routes/posts/router.ts"
export const postsRouter = new Hono().basePath("/posts").get(
  "/",
  validator(
    "query",
    z.object({
