...
- `adminProcedure` -> Admin endpoint, authentication and `admin.access` permission required
...
export const sayHelloToAuthenticatedUser = protectedProcedure.handler(
  async ({ input, context: { user, session } }) => {
    return {
...
For endpoints that are only available to admin users, you can use the `adminProcedure`. It additionally checks the `admin.access` permission via Permix (`@repo/permissions`).

```ts copy
import { adminProcedure } from "../orpc/procedures";

export const sayHelloToAdmin = adminProcedure.handler(
  async ({ input, context: { user, session } }) => {
    return {
      message: `Hello, Admin!`,
    };
...
Organization and payment procedures resolve membership for the target organization and use `checkPermission` for keys like `organization.manage` or `organization.accessBillingPortal`. See [Permissions and access control](/docs/nextjs/authentication/permissions) for the full matrix.
...
export const sayHelloToUser = publicProcedureWithSession.handler(
