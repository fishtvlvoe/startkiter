---
title: Accessing stored files
description: Learn how to access and download files stored in your supastarter application.
---

In the previous step you learned how to upload files to your storage provider.
At this point, these uploaded files are not accessible to anyone including the user that uploaded them.
...
```ts title="packages/api/src/routes/documents.ts"
...
If you to avoid creating signed URLs each time you can create a proxy endpoint that will automatically create a signed URL for the requested file.
This proxy endpoint should also include additional logic like checking if the user has access to the file or adding a cache layer.

```ts title="apps/saas/app/document-proxy/route.ts"
import { config as storageConfig } from "@repo/storage/config";
...
export const GET = async (
  ...
  if (bucket === storageConfig.bucketNames.documents) {
