...
The created products have to be defined in the `packages/payments/config.ts` file:

```js title="packages/payments/config.ts"
export const config = {
	plans: {
		plans: {
		pro: {
...
Set the `currency` value in the i18n configuration for the locales you support so the marketing and SaaS apps can resolve the correct price display for each locale.
