...
To deploy your application with Docker to [fly.io](http://fly.io) simply install the [Fly CLI](https://fly.io/docs/hands-on/install-flyctl/) and run the launch command for the app you are deploying.

For example, if you are deploying the SaaS app:

```bash
fly launch --dockerfile apps/saas/Dockerfile
```
