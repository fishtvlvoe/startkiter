...
## Add rewrite rule to the app that proxies the API

The API will be running on a different URL than the app. To not have to handle cross-origin requests, we are going to proxy the API requests through the app that serves your frontend.
To do this, add a rewrite rule to the Next.js app that should proxy the requests:

```ts title="apps/saas/next.config.ts"
...
**Note:** We are redirecting all requests to the `/api` path to the API service, except for the `/api/docs-search` path. This is because the docs search is a serverless function inside the docs app and we don't want to proxy it through the API service.
...
Keep the proxy route in the app that owns it, for example in the SaaS app if your product UI is proxying the API.
