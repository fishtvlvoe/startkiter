...
describe("POST /api/pages-cms/:id/restore", () => {
	...
	it("returns 401 when there is no session", async () => {
		...
		const response = await POST(new Request("http://localhost/api/pages-cms/page-missing/restore"), params);

		expect(response.status).toBe(401);
		expect(db.page.findUnique).not.toHaveBeenCalled();
	});

	it("returns 403 when a non-operator restores", async () => {
