			...
			<div className="flex items-center justify-between border-b pb-4">
				...
				<Card className="flex flex-wrap items-end gap-4 p-4">
					<div className="min-w-64 flex-1 space-y-1">
						...
						<select
							...
							className="w-full rounded-md border border-divider bg-surface px-3 py-2 text-sm"
						>
					...
					{isOperator ? (
						<>
							...
							<div className="min-w-72 space-y-1">
								...
								<div className="flex gap-2">
									<select
										...
										className="min-w-0 flex-1 rounded-md border border-divider bg-surface px-3 py-2 text-sm"
									>
							...
							{assignedInstructors.length > 0 ? (
								<div className="w-full basis-full border-t border-divider pt-3 text-xs">
									<span className="mr-3 text-caption">目前講師</span>
									{assignedInstructors.map((instructor) => (
										<span key={instructor.id} className="mr-2 inline-flex items-center gap-1 rounded bg-surface-hover px-2 py-1">
				...
				{isOperator && courseId ? (
					<Card className="space-y-3 p-4" data-testid="course-cover-settings">
						<div>
							<h2 className="text-base font-semibold text-heading">課程封面</h2>
							<p className="text-xs text-caption">上傳或選擇媒體庫圖片，公開課程頁會顯示最新封面。</p>
						</div>
						<MediaPicker
							type="IMAGE"
				...
				{isOperator && courseId ? (
					<Card className="space-y-4 p-4" data-testid="watermark-settings">
						<div className="flex items-center justify-between gap-3">
							<div>
								<h2 className="text-base font-semibold text-heading">影片浮水印</h2>
								<p className="text-xs text-caption">播放時顯示目前學員自己的識別資訊，位置會定時移動。</p>
							</div>
						...
						<div className="grid gap-4 md:grid-cols-4">
							<label className="space-y-1 text-xs text-caption">
								Email 顯示方式
								<select
									...
									className="w-full rounded-md border border-divider bg-surface px-3 py-2 text-sm text-heading"
								>
				...
				<div className="col-span-4 flex flex-col gap-4 overflow-y-auto border-r pr-4">
					...
					<div className="space-y-2">
						<div className="flex items-center justify-between">
							<span className="text-xs font-semibold uppercase text-caption">功能群組 / 資料夾</span>
								{isOperator ? (
						...
						{folders.map((f) => (
							<div key={f.id} className="rounded-lg border bg-surface/50 p-2 text-sm">
								<div className="flex items-center justify-between">
									<div
									...
									>
										...
										{editingFolderId === f.id ? (
											<input
												...
												className="rounded bg-surface-hover px-1 text-xs"
												autoFocus
											/>
										) : (
					...
					<div className="space-y-4 pt-2">
						<div className="flex items-center justify-between">
							<span className="text-xs font-semibold uppercase text-caption">課程單元編排</span>
						...
						{chapters.map((ch) => (
							<div key={ch.id} className="space-y-2">
								<div className="flex items-center justify-between rounded bg-surface-hover/80 px-2 py-1.5 text-xs font-semibold text-body">
									<span>{ch.title}</span>
									<button
					...
					{selectedLesson ? (
						<div className="space-y-6">
							...
							<Card className="space-y-4 p-4">
								<h2 className="text-base font-semibold text-heading">單元基本設定</h2>
							...
							<Card className="space-y-4 p-4">
								<div className="flex items-center justify-between gap-3">
									<h2 className="text-base font-semibold text-heading">內嵌工具（選填）</h2>
