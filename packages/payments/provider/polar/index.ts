...
export const createCheckoutLink: CreateCheckoutLink = async (options) => {
	...
	const { priceId, redirectUrl, customerId, organizationId, userId } = options;
...
export const webhookHandler: WebhookHandler = async (req) => {
	...
	try {
		...
		switch (event.type) {
			case "order.created": {
				const { metadata, customerId, subscription, productId } = event.data;
				...
				const planId = getPlanIdByProviderPriceId(productId);
			...
			case "subscription.created": {
				const { metadata, customerId, productId, id, status } = event.data;
				const planId = getPlanIdByProviderPriceId(productId);
