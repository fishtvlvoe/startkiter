...
const { store, sendEmailMock, assertConsentMock, nowRef } = vi.hoisted(() => {
	...
	return {
		store,
		sendEmailMock: vi.fn(async (_params: { signal?: AbortSignal }) => true),
		assertConsentMock: vi.fn(async (_userId: string, _type: string) => ({ allowed: true as boolean })),
				...
				for (const row of store.campaigns.values()) {
					...
					for (const [key, value] of Object.entries(data)) {
						if (key === "sentCount" || key === "failedCount" || key === "skippedCount" || key === "sentCursor") {
							(row as Record<string, unknown>)[key] = applyIncrement((row as unknown as Record<string, number>)[key], value);
						} else {
