...
MVP extract 佇列已清空：`extract-site-agent`、`extract-line-learner-community`、`extract-github-kit-fulfillment` 皆已封存。現行施工：`bootstrap-test-startkiter`（TEST repo／Vercel／Neon）。測試站：`https://test-startkiter.vercel.app`。kit 真測仍需老闆給 GITHUB_KIT_ORG／REPO／完整 PEM。
